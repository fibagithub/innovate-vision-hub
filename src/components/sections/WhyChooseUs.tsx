...
export function WhyChooseUs() {
  ...
  const reasons = [
    {
      icon: Rocket, title: t('why.innovation'), subtitle: t('why.innovationSub'),
      description: t('why.innovationDesc'), stat: "100+", statLabel: t('why.innovationStat'),
      gradient: "from-[#6366f1] to-[#8b5cf6]", delay: 0,
    },
    {
      icon: Shield, title: t('why.security'), subtitle: t('why.securitySub'),
      description: t('why.securityDesc'), stat: "24/7", statLabel: t('why.securityStat'),
      gradient: "from-[#059669] to-[#34d399]", delay: 100,
    },
    {
      icon: Zap, title: t('why.speed'), subtitle: t('why.speedSub'),
      description: t('why.speedDesc'), stat: "2x", statLabel: t('why.speedStat'),
      gradient: "from-[#f59e0b] to-[#fbbf24]", delay: 200,
    },
    {
      icon: Users, title: t('why.team'), subtitle: t('why.teamSub'),
      description: t('why.teamDesc'), stat: "15+", statLabel: t('why.teamStat'),
      gradient: "from-[#ec4899] to-[#f472b6]", delay: 300,
    },
  ];

  const stats = [
    { value: "50+", label: t('why.stat.clients'), icon: Users },
  ...
  const FeatureCard = ({ reason, index }: { reason: (typeof reasons)[0]; index: number }) => {
    ...
    return (
      <div className="group relative" onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)} style={{ animationDelay: `${reason.delay}ms` }}>
        <div className={`absolute -inset-0.5 rounded-3xl bg-gradient-to-r ${reason.gradient} opacity-0 group-hover:opacity-100 blur-xl transition-all duration-500`} />
        <div className="relative h-full p-8 rounded-3xl bg-card/80 backdrop-blur-xl border border-border/50 overflow-hidden transition-all duration-500 group-hover:border-transparent group-hover:shadow-2xl group-hover:-translate-y-2">
          ...
          <div className="relative z-10">
            ...
            <span className={`text-xs font-bold tracking-widest uppercase bg-gradient-to-r ${reason.gradient} bg-clip-text text-transparent`}>{reason.subtitle}</span>
            <h3 className="font-display text-2xl font-bold text-foreground mt-2 mb-3 group-hover:text-foreground transition-colors">{reason.title}</h3>
            ...
            <div className="flex items-end gap-3 pt-6 border-t border-border/50">
              <span className={`text-4xl font-display font-bold bg-gradient-to-r ${reason.gradient} bg-clip-text text-transparent`}>{reason.stat}</span>
              <span className="text-muted-foreground text-sm pb-1">{reason.statLabel}</span>
            </div>
          </div>
          <div className={`absolute -bottom-10 -right-10 w-32 h-32 rounded-full bg-gradient-to-r ${reason.gradient} opacity-10 group-hover:opacity-30 blur-2xl transition-all duration-500 group-hover:scale-150`} />
        </div>
      </div>
    );
  };
  ...
  return (
    <section className="py-32 bg-background relative overflow-hidden" id="why-us">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-gradient-to-r from-[#6366f1]/10 to-[#ec4899]/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: "1s" }} />
        ...
        <div className="max-w-4xl mx-auto text-center mb-20">
          ...
          <h2 className="font-display text-5xl sm:text-6xl lg:text-7xl font-bold text-foreground mb-8 tracking-tight leading-[1.1]">
            ...
            <span className="block mt-2">
              <span className="bg-gradient-to-r from-primary via-[#6366f1] to-[#ec4899] bg-clip-text text-transparent">{t('why.titleHighlight')}</span>
            </span>
          </h2>
          ...
          <div className="relative grid grid-cols-2 lg:grid-cols-4 gap-0 divide-x divide-white/20 p-8 lg:p-10">
            {stats.map((stat, index) => (
              <div key={index} className="text-center px-4 py-4 group cursor-default">
                ...
                <div className="text-4xl lg:text-5xl font-display font-bold text-white mb-2 group-hover:scale-105 transition-transform">{stat.value}</div>
        ...
        <div className="relative rounded-3xl overflow-hidden">
          ...
          <div className="relative p-10 lg:p-14 flex flex-col lg:flex-row items-center justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Link to="/services">
                <Button variant="outline" size="xl">{t('why.cta.viewServices')}</Button>
              </Link>
            </div>
          </div>
        </div>
