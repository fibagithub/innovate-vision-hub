...
import { ArrowRight, Database, FileText, Smartphone, CreditCard, BarChart3, Users, Zap, Shield, Globe, Sparkles, ExternalLink } from 'lucide-react';
...
const allProducts = [
  ...
  },
  {
    ...
    features: ['EMV чип', 'Contactless', 'QR төлбөр'],
...
const ProductLogo = ({ product, className = '' }: { product: typeof allProducts[0], className?: string }) => (
  <div className={`flex items-center justify-center rounded-2xl bg-gradient-to-br ${product.gradient} shadow-lg ${className}`}>
...
const ProductCard = ({ product, index }: { product: typeof allProducts[0], index: number }) => {
    ...
    return (
      <div className="col-span-12 lg:col-span-7">
        <CardWrapper {...linkProps as any} className="block h-full">
          <div className={`group relative h-full min-h-[480px] p-8 lg:p-10 rounded-[2rem] bg-gradient-to-br ${product.bgGradient} border border-border/50 overflow-hidden transition-all duration-700 hover:shadow-2xl hover:shadow-primary/10 hover:border-primary/20`}>
            {/* Animated Background */}
            ...
            <div className="relative h-full flex flex-col">
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-4">
                  <ProductLogo product={product} className="w-16 h-16 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500" />
                  <div>
                    <span className="text-xs font-bold tracking-widest text-primary/60 uppercase">{product.subtitle}</span>
  ...
  if (isLarge) {
    return (
      <div className="col-span-12 lg:col-span-5">
        <CardWrapper {...linkProps as any} className="block h-full">
          <div className={`group relative h-full min-h-[480px] p-8 rounded-[2rem] bg-gradient-to-br ${product.bgGradient} border border-border/50 overflow-hidden transition-all duration-500 hover:shadow-xl hover:border-${product.accentColor}/30`}>
            {/* Gradient Accent */}
            <div className={`absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r ${product.gradient}`} />
            
            {/* Decorative */}
            <div className={`absolute top-10 right-10 w-32 h-32 bg-gradient-to-br ${product.gradient} opacity-10 rounded-full blur-3xl`} />
            
            <div className="relative h-full flex flex-col">
              <div className="flex items-start justify-between mb-6">
                <ProductLogo product={product} className="w-14 h-14 group-hover:scale-110 transition-transform duration-500" />
                <span className="text-5xl font-black text-${product.accentColor}/10">0{index + 1}</span>
              </div>
              
              <span className={`text-xs font-bold tracking-widest text-${product.accentColor}/70 uppercase mb-2`}>{product.subtitle}</span>
              <h3 className={`font-display text-3xl lg:text-4xl font-bold text-foreground mb-4 group-hover:text-${product.accentColor} transition-colors`}>
                {product.title}
              </h3>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {product.features.map((feature, i) => (
                  <span key={i} className={`px-3 py-1.5 rounded-full bg-${product.accentColor}/5 border border-${product.accentColor}/10 text-xs font-medium text-foreground/80`}>
                    {feature}
                  </span>
                ))}
              </div>
  ...
  return (
    <div className="col-span-12 md:col-span-6 lg:col-span-4">
      <CardWrapper {...linkProps as any} className="block h-full">
        <div className={`group relative h-full min-h-[380px] p-8 rounded-[2rem] bg-gradient-to-br ${product.bgGradient} border border-border/50 overflow-hidden transition-all duration-500 hover:shadow-xl hover:border-${product.accentColor}/30`}>
          {/* Gradient Accent Line */}
          <div className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${product.gradient}`} />
          
          {/* Decorative Glow */}
          <div className={`absolute top-6 right-6 w-24 h-24 bg-gradient-to-br ${product.gradient} opacity-15 rounded-full blur-2xl group-hover:opacity-25 transition-opacity`} />
          
          <div className="relative h-full flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <ProductLogo product={product} className="w-14 h-14 group-hover:scale-110 group-hover:rotate-3 transition-all duration-500" />
            ...
            <span className={`text-xs font-bold tracking-widest uppercase mb-2`} style={{ color: `var(--${product.accentColor === 'primary' ? 'primary' : product.accentColor.replace('[', '').replace(']', '')})` }}>
              {product.subtitle}
            </span>
            <h3 className={`font-display text-2xl lg:text-3xl font-bold text-foreground mb-3 group-hover:text-${product.accentColor} transition-colors`}>
              {product.title}
            </h3>
            <p className="text-muted-foreground leading-relaxed flex-1 mb-6">
              {product.description}
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {product.features.map((feature, i) => (
                <span key={i} className="px-3 py-1.5 rounded-full bg-foreground/5 border border-border/50 text-xs font-medium text-foreground/70">
                  {feature}
                </span>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between pt-4 border-t border-border/50">
              <div className="flex items-center gap-2">
                <Sparkles className="w-4 h-4" style={{ color: `var(--${product.accentColor === 'primary' ? 'primary' : product.accentColor.replace('[', '').replace(']', '')})` }} />
                <span className="text-sm font-semibold" style={{ color: `var(--${product.accentColor === 'primary' ? 'primary' : product.accentColor.replace('[', '').replace(']', '')})` }}>
                  {product.stats}
                </span>
              </div>
              <ArrowRight className={`w-5 h-5 text-muted-foreground group-hover:text-${product.accentColor} group-hover:translate-x-1 transition-all`} />
            </div>
          </div>
        </div>
      </CardWrapper>
